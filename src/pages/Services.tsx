    ...
    <>
      ...
      <section className="mx-auto max-w-shell px-[22px] pt-10">
        <div className="max-w-[820px]">
          <div className="text-[13px] uppercase tracking-[.18em]" style={{ color: '#7dd3fc' }}>Services</div>
      ...
      {columns.map((col) => (
        <Section key={col.title} heading={col.title}>
          <div className="grid gap-3 mt-6" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))' }}>
            {col.links.map((l) => (
              <Link key={l.label} to={l.to} className="p-4 rounded-[18px] font-semibold text-[16px] !text-paper" style={glass}>
                {l.label} →
              </Link>
            ))}
          </div>
