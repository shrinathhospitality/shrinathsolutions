...
export default function Hero({ kicker, h1, intro, ctaLabel, aside, notes }: Props) {
  return (
    <section className="mx-auto max-w-shell px-[22px] pt-10 grid gap-11 items-center" style={{ gridTemplateColumns: aside ? 'repeat(auto-fit, minmax(330px, 1fr))' : '1fr' }}>
      <div>
        <div className="text-[13px] uppercase tracking-[.18em]" style={{ color: 'var(--color-primary)' }}>{kicker}</div>
        <motion.h1
        ...
        <p className="text-[18.5px] mt-5 max-w-[660px]" style={{ color: 'var(--color-body)' }}>{intro}</p>
        ...
        {notes && (
          <div className="flex flex-wrap gap-6 mt-7 text-[14.5px]" style={{ color: 'var(--color-body)' }}>
