...
export default function SeoMidCta({ heading, body, buttonLabel }: { heading: string; body: string; buttonLabel: string }) {
  return (
    <section className="mx-auto max-w-shell px-[22px] pt-16">
      <div className="flex flex-col md:flex-row items-center gap-6 justify-between p-6 md:p-7 rounded-[20px]" style={{ border: '1px solid rgba(255,255,255,.11)', background: 'rgba(255,255,255,.035)' }}>
        <div className="flex items-center gap-4">
          <span className="grid place-items-center rounded-full shrink-0" style={{ width: 46, height: 46, background: 'rgba(255,122,47,.16)' }}>
            <Target size={20} color="#ff9a53" aria-hidden="true" />
          </span>
          <div>
            <div className="font-heading font-bold text-[18px]">{heading}</div>
            <p className="m-0 mt-1 text-[14.5px]" style={{ color: 'rgba(226,234,255,.7)' }}>{body}</p>
          </div>
        </div>
