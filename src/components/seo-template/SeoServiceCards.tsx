        ...
        {cards.map((c, i) => {
          const content = (
            <>
              <span className="grid place-items-center" style={{ width: 42, height: 42, borderRadius: 13, background: 'var(--color-surface-alt)', border: '1px solid var(--color-border)' }}>
                <c.icon size={19} color="var(--color-primary)" aria-hidden="true" />
              </span>
          ...
          return (
            <motion.div key={c.title} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.4, delay: i * 0.06 }}>
              {c.to ? (
                <Link to={c.to} className="group block h-full p-6 rounded-[20px] !text-heading transition-all hover:-translate-y-0.5" style={glass}>
                  {content}
                </Link>
              ) : (
